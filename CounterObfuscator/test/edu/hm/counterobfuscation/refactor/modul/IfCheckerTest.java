...
public class IfCheckerTest {
	...
	@Test
	public void ifCheckerTest1() throws IllegalArgumentException, IOException, EncoderException, ScriptException {
		
		String input = "if(false){var test=0;}";
		
		IParser parser = ParserFactory.create(input, false);
				
		IModul ifcheckerModul = new IfChecker(parser.getProgrammTree(), client);
		IProgrammTree tree = ifcheckerModul.process(); 
				
		assertEquals(0, tree.size());	
	}
	
	@Test
	public void ifCheckerTest2() throws IllegalArgumentException, IOException, EncoderException, ScriptException {
		
		String input = "if(true){var test=0;}";
